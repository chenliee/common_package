...
class WelfareModel {
  WelfareModel({
    List<PositionItem>? welfareAdvert,
    List<PositionItem>? registrationGiftAdvert,
    List<GoodItem>? goodList,
    List<SpikeItem>? secKillActList,
  }) {
    _welfareAdvert = welfareAdvert;
    _registrationGiftAdvert = registrationGiftAdvert;
    _goodList = goodList;
    _secKillActList = secKillActList;
  }

  WelfareModel.fromJson(dynamic json) {
    if (json['welfareAdvert'] != null) {
      _welfareAdvert = [];
      json['welfareAdvert'].forEach((v) {
        _welfareAdvert?.add(PositionItem.fromJson(v));
      });
    }
    if (json['registrationGiftAdvert'] != null) {
      _registrationGiftAdvert = [];
      json['registrationGiftAdvert'].forEach((v) {
        _registrationGiftAdvert?.add(PositionItem.fromJson(v));
      });
    }
  ...
  List<PositionItem>? _welfareAdvert;
  List<PositionItem>? _registrationGiftAdvert;
  List<GoodItem>? _goodList;
  List<SpikeItem>? _secKillActList;
  WelfareModel copyWith({
    List<PositionItem>? welfareAdvert,
    List<PositionItem>? registrationGiftAdvert,
    List<GoodItem>? goodList,
    List<SpikeItem>? secKillActList,
      ...
      WelfareModel(
        welfareAdvert: welfareAdvert ?? _welfareAdvert,
        registrationGiftAdvert:
            registrationGiftAdvert ?? _registrationGiftAdvert,
        goodList: goodList ?? _goodList,
        secKillActList: secKillActList ?? _secKillActList,
      );
  List<PositionItem>? get welfareAdvert => _welfareAdvert;
  List<PositionItem>? get registrationGiftAdvert => _registrationGiftAdvert;
  List<GoodItem>? get goodList => _goodList;
  List<SpikeItem>? get secKillActList => _secKillActList;

  Map<String, dynamic> toJson() {
    final map = <String, dynamic>{};
    if (_welfareAdvert != null) {
      map['welfareAdvert'] = _welfareAdvert?.map((v) => v.toJson()).toList();
    }
    if (_registrationGiftAdvert != null) {
      map['registrationGiftAdvert'] =
          _registrationGiftAdvert?.map((v) => v.toJson()).toList();
    }
    if (_goodList != null) {
